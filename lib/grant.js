 ...
 * @param {Function} next
 */
function Grant (config, ctx, req, res, next) {
  this.config = config;
...
function checkClient (done) {
  var self = this;
  this.model.getClient(this.ctx, this.client.clientId, this.client.clientSecret,
      function (err, client) {
...
function useAuthCodeGrant (done) {
  ...
  var self = this;
  this.model.getAuthCode(this.ctx, code, function (err, authCode) {
    if (err) return done(error('server_error', false, err));
...
function usePasswordGrant (done) {
  ...
  var self = this;
  return this.model.getUser(this.ctx, uname, pword, function (err, user) {
    if (err) return done(error('server_error', false, err));
    if (!user) {
...
function useRefreshTokenGrant (done) {
  ...
  var self = this;
  this.model.getRefreshToken(this.ctx, token, function (err, refreshToken) {
    if (err) return done(error('server_error', false, err));
    ...
    if (self.model.revokeRefreshToken) {
      return self.model.revokeRefreshToken(this.ctx, token, function (err) {
        if (err) return done(error('server_error', false, err));
        done();
      });
    }
...
function useClientCredentialsGrant (done) {
  ...
  var self = this;
  return this.model.getUserFromClient(this.ctx, clientId, clientSecret,
      function (err, user) {
    if (err) return done(error('server_error', false, err));
    if (!user) {
...
function useExtendedGrant (done) {
  var self = this;
  this.model.extendedGrant(this.ctx, this.grantType, this.req,
...
function checkGrantTypeAllowed (done) {
  this.model.grantTypeAllowed(this.ctx, this.client.clientId, this.grantType,
...
function saveAccessToken (done) {
  ...
  this.model.saveAccessToken(this.ctx, accessToken, this.grantType,
      this.client.clientId, this.req.oauth.client.accessTokenLifetime, this.user, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
}
...
function saveRefreshToken (done) {
  ...
  this.model.saveRefreshToken(this.ctx, refreshToken, this.grantType, this.client.clientId, this.req.oauth.client.refreshTokenLifetime,
...
function sendResponse (done) {
  ...
  if (this.req.oauth.client.accessTokenLifetime) {
    response.expires_in = this.req.oauth.client.accessTokenLifetime;
  }
