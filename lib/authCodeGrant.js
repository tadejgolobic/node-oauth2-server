 ...
 * @param {Function} next
 */
function AuthCodeGrant(config, ctx, req, res, next, check) {
  this.config = config;
...
function checkClient (done) {
  var self = this;
  this.model.getClient(this.ctx, this.clientId, null, function (err, client) {
...
function saveAuthCode (done) {
  ...
  this.model.saveAuthCode(this.ctx, this.authCode, this.client.clientId, expires,
...
function saveAccessToken (done) {
  ...
  this.model.saveAccessToken(this.ctx, accessToken, 'auth_code',
    this.client.clientId, this.client.accessTokenLifetime, this.user, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
}
...
function saveRefreshToken (done) {
  ...
  this.model.saveRefreshToken(this.ctx, refreshToken, 'implicit', this.client.clientId, this.client.refreshTokenLifetime,
      this.user, function (err) {
    if (err) return done(error('server_error', false, err));
    done();
  });
...
function redirect (done) {
  ...
  if(this.responseType === 'code') {
    ...
    url = this.client.redirectUri + '?access_token=' + this.accessToken + '&refresh_token=' + this.refreshToken + "&expires_in=" + this.client.accessTokenLifetime +'&token_type=bearer';
  }
